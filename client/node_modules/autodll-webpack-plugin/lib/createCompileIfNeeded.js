...
var createCompileIfNeeded = function createCompileIfNeeded(log, settings) {
    return function (getCompiler) {
        return (0, _validateCache2.default)(settings).then(log.tap(function (isValid) {
            return `is valid cache? ${isValid}`;
        })).then(function (isValid) {
            if (isValid) return null;
            return Promise.resolve().then(log.tap('cleanup')).then(emptyCacheDir(settings)).then(log.tap('compile')).then(runCompile(settings, getCompiler)).then(function (stats) {
                return stats;
            });
        });
    };
