...
// Public methods
    ...
    add(paths_, _origAdd, _internal) {
        ...
        >
            {
                const res = await
                this._nodeFsHandler._addToNodeFs(path, !_internal, 0, 0, _origAdd);
    ...
    unwatch(paths_) {
        ...
        paths.forEach((path) = > {
            ...
            if(
        !sysPath.isAbsolute(path) && !this._closers.has(path)
    )
        {
    ...
    close() {
        ...
        ;
        return closers.length ? Promise.all(closers).then(() = > undefined
    ) :
        Promise.resolve();
    }
