...
function transformAST(node, opts) {
    ...
    if (child.nodes && Object(child.nodes).length) {
...
function transformArgsByParams(node, params) {
    ...
    return params.map(param = > nodes.map((childNode, index) = > typeof param[index] === 'function' ? param[index](childNode, opts) : undefined
).
    filter(child = > typeof child !== 'boolean'
)).
    filter(param = > param.every(result = > result !== undefined
))
    [0] || [];
}
