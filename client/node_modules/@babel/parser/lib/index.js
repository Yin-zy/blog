        ...
        switch (expr.type) {
            case "ObjectPattern":
                expr.properties.forEach(prop = > {
                    this
                    .checkLVal(prop.type === "Property" ? prop.value : prop, bindingType, checkClashes, "object destructuring pattern", disallowLetBinding);
        }
    )
        ;
        break;
...
var flow = (superClass = > class extends superClass {
    ...
    flowParseDeclareModule(node) {
        ...
        if (bodyElement.type === "DeclareModuleExports") {
            if (hasModuleExport) {
    ...
    parseNewArguments(node) {
        ...
        if (this.shouldParseTypes() && this.isRelational("<")) {
            targs = this.tryParse(() = > this.flowParseTypeParameterInstantiationCallOrNew()
        ).
            node;
        }
...
var typescript = (superClass = > class extends superClass {
    ...
    tsParseBindingListForSignature() {
        return this.parseBindingList(types.parenR, 41).map(pattern = > {
            if(pattern
            .type !== "Identifier" && pattern.type !== "RestElement" && pattern.type !== "ObjectPattern" && pattern.type !== "ArrayPattern"
    )
        {
    ...
    tsParseTupleType() {
        ...
        if (seenOptionalElement && elementNode.type !== "TSRestType") {
    ...
    parseSubscript(base, startPos, startLoc, noCalls, state) {
        ...
        if (this.isRelational("<")) {
            const result = this.tsTryParseAndCatch(() = > {
                if(
            !noCalls && this.atPossibleAsync(base)
        )
            {
...
...
    ...
    const
    typeParameters = this.tsTryParseTypeParameters();
...
)
{
    ...
    if (this.match(types.jsxTagStart)) {
        state = this.state.clone();
        jsx = this.tryParse(() = > super.parseMaybeAssign(...args
    ),
        state
    )
        ;
