...
const validBrowserslistTargets = [...Object.keys(_browserslist.default.data), .
..
...
>
{
    ...
    if (targets.esmodules) {
        ...
        targets.browsers = Object.keys(supportsESModules).map(browser = > `${browser} ${supportsESModules[browser]}`
    ).
        join(", ");
    }
    ...
    const parsed = Object.keys(targets).filter(value = > value !== _options.TargetNames.esmodules
).
    sort().reduce((results, target) = > {
