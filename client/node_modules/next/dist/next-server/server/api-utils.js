...
function apiResolver(req, res, params, resolverModule, onError) {
    const apiReq = req;
    const apiRes = res;
    try {
        let config = {};
        ...
        if (bodyParser) {
            apiReq.body = await
            parseBody(apiReq, config.api && config.api.bodyParser && config.api.bodyParser.sizeLimit
                ? config.api.bodyParser.sizeLimit
                : '1mb');
        }
        apiRes.status = statusCode =
    >
        sendStatusCode(apiRes, statusCode);
        apiRes.send = data =
    >
        sendData(apiRes, data);
        apiRes.json = data =
    >
        sendJson(apiRes, data);
...
function parseBody(req, limit) {
    ...
    try {
        buffer = await
        raw_body_1.default(req, {encoding, limit});
    } catch (e) {
