...
function getSprCache(pathname) {
    ...
    if (!data) {
        try {
            const html = await
            readFile(getSeedPath(pathname, 'html'), 'utf8');
            const pageData = JSON.parse(await
            readFile(getSeedPath(pathname, 'json'), 'utf8')
        )
            ;
            data = {
                html,
...
function setSprCache(pathname, data, revalidateSeconds) {
    ...
    if (sprOptions.flushToDisk) {
        try {
            ...
            await
            mkdirp(path_1.default.dirname(seedPath));
            await
            writeFile(seedPath, data.html, 'utf8');
            await
            writeFile(getSeedPath(pathname, 'json'), JSON.stringify(data.pageData), 'utf8');
        } catch (error) {
